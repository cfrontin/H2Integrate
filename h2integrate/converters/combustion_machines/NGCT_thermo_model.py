...
DRY_AIR_MASS_FRACTIONS = {
    pyfluids.FluidsList.Nitrogen: 75.51760366879898,  # generated by copilot, matches engineeringtoolbox.com data
    pyfluids.FluidsList.Oxygen: 23.139561050152018,  # generated by copilot, matches engineeringtoolbox.com data
    pyfluids.FluidsList.Argon: 1.2881375564363484,  # generated by copilot, matches engineeringtoolbox.com data
    pyfluids.FluidsList.CarbonDioxide: 0.05469772461264626,  # generated by copilot, matches engineeringtoolbox.com data
}
...
def enforce_gas_phase(fluid):
    ...
    try:
        ...
        # Keep behavior backward compatible if phase forcing is not supported.
        pass
    return fluid
...
def compute_compressor_outlet_state_isentropic(
    ...
    state_outlet = state_inlet.with_state(
        ...
        pyfluids.Input.entropy(
            state_inlet.entropy
        ),  # by isentropic compression assumption
    )
...
def compute_heat_transfer(
    ...
    return mass_flow_fluid * (
        state_outlet.enthalpy / 1.0e3 - state_inlet.enthalpy / 1.0e3
    )  # kJ/s
...
def compute_turbine_outlet_state_isentropic(state_inlet, ratio_P):
    ...
    state_outlet = state_inlet.with_state(
        ...
        pyfluids.Input.entropy(
            state_inlet.entropy
        ),  # by isentropic expansion assumption
    )
...
class NGCTResult:
    ...
    def get_net_heat_input(self):
        mass_flowrate = self.mass_flowrate if self.mass_flowrate else 1.0
        return mass_flowrate * sum(
            np.maximum(0.0, list(self.process_heat_unit.values()))
        )  # kJ/s

    def get_net_heat_rejection(self):
        mass_flowrate = self.mass_flowrate if self.mass_flowrate else 1.0
        return mass_flowrate * sum(
            np.minimum(0.0, list(self.process_heat_unit.values()))
        )  # kJ/s

    def get_back_work_ratio(self):
        back_work = -sum(
            np.minimum(0.0, list(self.process_work_unit.values()))
        )  # kJ/s
        forward_work = sum(
            np.maximum(0.0, list(self.process_work_unit.values()))
        )  # kJ/s
    ...
    def get_fuel_mass_flow(self, LHV_fuel, heating_process=(2, 3)):
        ...
        mass_flowrate_fuel = combustor_heat_input_rate / (
            1000.0 * LHV_fuel
        )  # [J/s]/[J/kg] = kg/s
    ...
    def __str__(self):
        ...
        for idx_state, state in self.states.items():
            ...
            output += f"\t{idx_state} ({state_name}): T={state.temperature:.2f} C, h={state.enthalpy/1.0e3:.2f} MJ/kg, P={state.pressure/1.0e3:.2f} kPa\n"
        ...
        for (idx_in, idx_out), work_unit in self.process_work_unit.items():
            ...
            output += f"\t{idx_in} -> {idx_out} ({process_name}): W={work_unit:.2f} kJ/kg, Q={heat_unit:.2f} kJ/kg\n"
...
class NGCT:
    ...
    Thermodynamic model for the performance of a specific natural gas combustion turbine.
    We assume a Brayton cycle with isentropic efficiency corrections for the compressor and turbine work.
    The turbine is assumed to ingest the intake fluid at maximum a specific volumetric flowrate.
    We also assume that the combustion chamber and turbine set the maximum designed firing temperature.
    """
    ...
    def __init__(
        ...
        isentropic_efficiency_compressor: float,  # -, compressor isentropic efficiency
        isentropic_efficiency_turbine: float,  # -, turbine isentropic efficiency
        Q_fluid_max: float = None,  # m**3/s, design volumetric flowrate; allows unit-mass analysis (units) if None
    ...
    def run_turbine_model(
        self,
        fluid_ambient_list: list[pyfluids.fluids.abstract_fluid.AbstractFluid] | pyfluids.fluids.abstract_fluid.AbstractFluid,
        ...
        for fluid_ambient in fluid_ambient_list:  # loop over ambient fluid states
            ...
            enforce_gas_phase(
                fluid_ambient
            )  # ensure the ambient fluid is in the gas phase
            ...
            enforce_gas_phase(
                fluid_compressed
            )  # ensure the compressed fluid is in the gas phase
            ...
            enforce_gas_phase(
                fluid_combusted
            )  # ensure the combusted fluid is in the gas phase
            ...
            enforce_gas_phase(
                fluid_exhaust
            )  # ensure the exhaust fluid is in the gas phase
            ...
            mass_flowrate = NGCT.get_mass_flowrate(
                fluid_ambient, self.Q_fluid_max,
                wdot_turbine, wdot_compressor, qdot_HX_combustor,
                power_rated=power_rated, heatrate_fuel_capacity=heatrate_fuel_capacity,
            ) if self.Q_fluid_max is not None else None  # use static method
    ...
    def get_mass_flowrate(
        ...
        Determines the actual mass flowrate as the minimum of multiple limiting cases:
        volumetric flow capacity, power rating, and heat rate capacity.

        Args:
            fluid_ambient: Ambient fluid mixture with density property (pyfluids.fluids.abstract_fluid.AbstractFluid)
        ...
        # print(f"DEBUG!!!!! {'flowrate limited' if np.argmin(mass_flowrate_candidates) == 0 else 'power limited' if np.argmin(mass_flowrate_candidates) == 1 else 'fuel limited'}")
        # if np.argmin(mass_flowrate_candidates) == 1:
        #     print(f"\tDEBUG!!!!! power_rated: {power_rated}")
        #     print(f"\tDEBUG!!!!! wdot_turbine: {wdot_turbine} wdot_compressor: {wdot_compressor}")
        #     print(f"\tDEBUG!!!!! power-set mass flowrate: {power_rated/(wdot_turbine + wdot_compressor)}")
...
class GE_7FA05_NGCT(NGCT):
    ...
    A thermodynamic model for the performance of the GE 7FA.05 natural gas combustion turbine in simple-cycle operation.
    We use a mix of fact- and data-sheet performance descriptions, textbook assumptions, and reverse-engineering to characterize the system.
    Reverse engineering targets matching the ISO performance at the design conditions.
    """
    ...
    def __init__(self):
        ...
        Q_fluid = (
            477.78734437019483  # m**3/s, by reverse-engineering from datasheet datasheet
        )
...
class GE_7EA_NGCT_2014(NGCT):
    ...
    A thermodynamic model for the performance of the GE 7EA natural gas combustion turbine in simple-cycle operation.
    We use a mix of fact- and data-sheet performance descriptions, textbook assumptions, and reverse-engineering to characterize the system.
    Reverse engineering targets matching the ISO performance at the design conditions.
...
if __name__ == "__main__":
    ...
    working_fluid = make_humid_air_mixture(
        P_ambient, Trel_ambient, rel_humidity_ambient
    )
